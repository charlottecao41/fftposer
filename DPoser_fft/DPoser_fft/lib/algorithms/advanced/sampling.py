...
class EulerMaruyamaPredictor(Predictor):
    ...
    def update_fn(self, x, t, observation, mask):
        ...
        x = x.reshape(B,(21//2+1)*3)
        drift, diffusion = self.rsde.sde(x, t)
        x_mean = orig_x + drift * dt
        ...
        # x_mean = x_mean.reshape(B,21,3)
        # x_mean = torch.fft.rfft(x_mean, dim = 1)
        # x_mean = x_mean.reshape(B,(21//2+1)*3)
        return x, x_mean
...
class LangevinCorrector(Corrector):
    ...
    def update_fn(self, x, t, observation, mask):
        ...
        B,D = x.shape
        x_fft = x.reshape(B,21,3)
        x_fft = torch.fft.rfft(x_fft, dim = 1)
        x_fft = x_fft.reshape(B,(21//2+1)*3)
        ...
        for i in range(n_steps):
            grad = score_fn(x_fft, t, condition=None, mask=mask)
            noise = torch.randn_like(x)
            grad_norm = torch.norm(grad.reshape(grad.shape[0], -1), dim=-1).mean()
            noise_norm = torch.norm(noise.reshape(noise.shape[0], -1), dim=-1).mean()
            step_size = (target_snr * noise_norm / grad_norm) ** 2 * 2 * alpha

            grad = grad.reshape(B,11,3)
            grad = torch.fft.irfft(grad, dim = 1, n= 21)
            grad = grad.reshape(B,63)
