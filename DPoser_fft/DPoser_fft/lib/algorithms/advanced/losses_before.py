  ...
  """
    ...
    """
        ...
        def multi_step_denoise(x_t, t, t_end, N=10):
            ...
            score = score.reshape(B,11,3)
            score = torch.fft.irfft(score, dim = 1, n = 21)
            score = score.reshape(B,63)

            x_current = x_current.reshape(B,11,3)
            x_current = torch.fft.irfft(x_current, dim = 1, n = 21)
            x_current = x_current.reshape(B,63)
            return score_return, x_current

        def one_step_denoise(sde, x_t, t):
            probability_flow = True
            rsde = sde.reverse(score_fn, probability_flow)
            drift, diffusion, alpha, sigma_2, score = rsde.sde(x_t, t, guide=True)
            x_0_hat = (x_t + sigma_2[:, None] * score) / alpha
            SNR = alpha / torch.sqrt(sigma_2)[:, None]

            x_0_hat = x_0_hat.reshape(B,11,3)
            x_0_hat = torch.fft.irfft(x_0_hat, dim = 1, n = 21)
            x_0_hat = x_0_hat.reshape(B,63)
        ...
        #change z
        z_prime = z.reshape(B,21,3)
        z_prime = torch.fft.rfft(z_prime, dim = 1)
        z_prime = z_prime.reshape(B,(21//2+1)*3)

        # perturbed_data = mean + std[:, None] * z  # [B, 33]
        # z = torch.randn_like(perturbed_data)  # [B, j*3]

        perturbed_data = batch
        perturbed_data = perturbed_data.reshape(B,21,3)
        perturbed_data = torch.fft.rfft(perturbed_data, dim = 1)
        perturbed_data = perturbed_data.reshape(B,(21//2+1)*3)

        mean, std = sde.marginal_prob(perturbed_data, t)

        perturbed_data = mean + std[:, None] * z_prime  # [B, 33]
        ...
        # denoise_data, SNR = one_step_denoise(sde, perturbed_data, t)
        weighted = False
        if weighted:
        ...
        # loss = 0.8*torch.mean(weight * nn.MSELoss(reduction='none')(batch, denoise_data)) + loss
