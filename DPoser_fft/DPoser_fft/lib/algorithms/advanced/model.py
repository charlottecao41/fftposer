...
class ScoreModelFC(nn.Module):
    ...
    def __init__(self, config, n_poses=21, pose_dim=6, hidden_dim=64,
        ...
        self.shared_time_embed = nn.Sequential(
            nn.Linear(embed_dim, embed_dim,dtype=torch.cfloat),
            self.act,
        )

        # self.shared_time_embedr = nn.Sequential(
        ...
        for idx in range(n_blocks):
            setattr(self, f'b{idx + 1}_dense1', nn.Linear(hidden_dim, hidden_dim,dtype=torch.cfloat))
            setattr(self, f'b{idx + 1}_dense1_t', nn.Linear(embed_dim, hidden_dim,dtype=torch.cfloat))
            setattr(self, f'b{idx + 1}_gnorm1', ComplexGnorm(nn.GroupNorm(32, num_channels=hidden_dim)))

            setattr(self, f'b{idx + 1}_dense2', nn.Linear(hidden_dim, hidden_dim,dtype=torch.cfloat))
            setattr(self, f'b{idx + 1}_dense2_t', nn.Linear(embed_dim, hidden_dim,dtype=torch.cfloat))
            setattr(self, f'b{idx + 1}_gnorm2', ComplexGnorm(nn.GroupNorm(32, num_channels=hidden_dim)))

        # for idx in range(n_blocks):
        #     setattr(self, f'b{idx + 1}_dense1r', nn.Linear(hidden_dim, hidden_dim))
        #     setattr(self, f'b{idx + 1}_dense1_tr', nn.Linear(embed_dim, hidden_dim))
        #     setattr(self, f'b{idx + 1}_gnorm1r', nn.GroupNorm(32, num_channels=hidden_dim))

        #     setattr(self, f'b{idx + 1}_dense2r', nn.Linear(hidden_dim, hidden_dim))
        #     setattr(self, f'b{idx + 1}_dense2_tr', nn.Linear(embed_dim, hidden_dim))
        #     setattr(self, f'b{idx + 1}_gnorm2r', nn.GroupNorm(32, num_channels=hidden_dim))
